...
#define __CPU_CAPACITY 15000000 // 10M
#endif

#ifndef __RNIC_CAPACITY
#define __RNIC_CAPACITY 40000000
#endif
...
// message unit must be 64B because RDMA Write only guarantees ordering within one cacheline
#ifndef __COUNTER_PER_MESG
#define __COUNTER_PER_MESG ((64 - sizeof(uint64_t)) / sizeof(counter_))
#endif

// there are __MAX_COUNTER_NUM * __SLOT_GRANULARITY entries
...
class Agent {
    public:
	// communication unit between agents, 64B
	typedef struct {
	...
	// send application rpc address etc. to agent client
