...
//
// InlineSkipList is derived from SkipList (index_.h), but it optimizes
...
class Random {
	...
	static Random *GetTLSInstance()
	{
		...
		thread_local std::aligned_storage<sizeof(Random)>::type
			tls_instance_bytes;
...
template <class Comparator> class InlineSkipList {
    private:
	struct Node;
	struct Splice;
	...
	// Create a new InlineSkipList object that will use "comparator_" for comparing
	...
	class Iterator {
		...
		// Change the underlying index_ used for this iterator
	...
	void FindSpliceForLevel(const DecodedKey &key, Node *before,
...
template <class Comparator> struct InlineSkipList<Comparator>::Splice {
...
template <class Comparator> struct InlineSkipList<Comparator>::Node {
...
template <class Comparator>
inline void
InlineSkipList<Comparator>::Iterator::SetList(const InlineSkipList *list)
{
	list_ = list;
	node_ = nullptr;
}
...
template <class Comparator>
inline void
InlineSkipList<Comparator>::Iterator::SeekForPrev(const char *target)
{
	Seek(target);
	if (!Valid()) {
		SeekToLast();
...
template <class Comparator> int InlineSkipList<Comparator>::RandomHeight()
{
					 ...
					 int bottom_level) const
{
	...
	while (true) {
		...
		if (next != last_not_after &&
		    KeyIsAfterNode(key_decoded, next)) {
			// Keep searching in this list
			assert(next != nullptr);
			x = next;
		} else {
...
uint64_t InlineSkipList<Comparator>::EstimateCount(const char *key) const
{
	...
	while (true) {
		...
		if (next == nullptr ||
		    compare_(next->Key(), key_decoded) >= 0) {
			if (level == 0) {
				return count;
			} else {
				// Switch to next list
...
InlineSkipList<Comparator>::InlineSkipList(const Comparator cmp,
	...
	: kMaxHeight_(static_cast<uint16_t>(max_height)),
	  ...
	  allocator_(allocator), compare_(cmp),
	  head_(AllocateNode(0, max_height)), max_height_(1),
...
InlineSkipList<Comparator>::AllocateSplice()
{
	...
	splice->next_ =
		reinterpret_cast<Node **>(raw + sizeof(Splice) + array_size);
	return splice;
}
						       ...
						       int recompute_level)
{
	...
	for (int i = recompute_level - 1; i >= 0; --i) {
		FindSpliceForLevel<true>(key, splice->prev_[i + 1],
					 splice->next_[i + 1], i,
					...
					bool allow_partial_splice_fix)
{
	...
	if (splice->height_ < max_height) {
		...
		while (recompute_height < max_height) {
			if (splice->prev_[recompute_height]->Next(
				    recompute_height) !=
				...
				if (allow_partial_splice_fix) {
					...
					Node *bad =
						splice->prev_[recompute_height];
					while (splice->prev_[recompute_height] ==
					       bad) {
						++recompute_height;
					}
				} else {
				...
				if (allow_partial_splice_fix) {
					Node *bad =
						splice->next_[recompute_height];
					while (splice->next_[recompute_height] ==
					       bad) {
						++recompute_height;
					}
				} else {
	...
	if (UseCAS) {
		for (int i = 0; i < height; ++i) {
			while (true) {
				...
				FindSpliceForLevel<false>(key_decoded,
							  splice->prev_[i],
							  nullptr, i,
							  &splice->prev_[i],
							  &splice->next_[i]);
...
void InlineSkipList<Comparator>::TEST_Validate() const
{
	...
	while (nodes[0] != nullptr) {
		...
		assert(nodes[0] == head_ ||
		       compare_(nodes[0]->Key(), l0_next->Key()) < 0);
		nodes[0] = l0_next;
