...
JsonConfig JsonConfig::load_file(std::string path)
{
	...
	if (!ifs.is_open()) {
		...
		assert(false);
		return JsonConfig(nullptr, nullptr,
				  std::string() + "<" + path + ">");
	}
...
JsonConfig JsonConfig::load(std::string json_text, std::string path)
{
	...
	if (root->HasParseError()) {
		fprintf(stderr, "error parsing config: %s (offset_=%zu)\n",
...
ALWAYS_INLINE
std::string JsonConfig::get_path() const
{
	return path_;
}

ALWAYS_INLINE
bool JsonConfig::exists() const
{
	return current_ != nullptr;
}

ALWAYS_INLINE
bool JsonConfig::is_bool() const
{
	return exists() && current_->IsBool();
}

ALWAYS_INLINE
bool JsonConfig::is_int64() const
{
	return exists() && current_->IsInt64();
}

ALWAYS_INLINE
bool JsonConfig::is_uint64() const
{
	return exists() && current_->IsUint64();
}

ALWAYS_INLINE
bool JsonConfig::is_double() const
{
	return exists() && current_->IsDouble();
}

ALWAYS_INLINE
bool JsonConfig::is_str() const
{
	return exists() && current_->IsString();
}

ALWAYS_INLINE
bool JsonConfig::is_array() const
{
	return exists() && current_->IsArray();
}

ALWAYS_INLINE
bool JsonConfig::is_dict() const
{
	return exists() && current_->IsObject();
}
...
JsonConfig JsonConfig::get(size_t index)
{
	...
	if (index >= current_->Size())
		...
		return JsonConfig(
			root_, &((*current_)[static_cast<unsigned int>(index)]),
			oss.str());
}
