...
template <typename BaseAllocator = CrtAllocator> class MemoryPoolAllocator {
	...
	/*! \param chunkSize The size of memory chunk. The default is kChunkSize.
  ...
  */
	MemoryPoolAllocator(void *buffer, size_t size,
		...
		: chunkHead_(0), chunk_capacity_(chunkSize),
		  userBuffer_(buffer), baseAllocator_(baseAllocator),
		  ownBaseAllocator_(0)
	{
  ...
  */
	...
	void *Malloc(size_t size)
	{
		...
		if (chunkHead_ == 0 ||
		    chunkHead_->size + size > chunkHead_->capacity)
			AddChunk(chunk_capacity_ > size ? chunk_capacity_ :
							  size);
	...
	void *Realloc(void *originalPtr, size_t originalSize, size_t newSize)
	{
		...
		RAPIDJSON_ASSERT(newBuffer !=
				 0); // Do not handle out-of-memory explicitly.
		if (originalSize)
