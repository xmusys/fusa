...
#include "reader.h"
#include "util/rapidjson/internal/meta.h"
#include "util/rapidjson/internal/strfunc.h"
...
struct IsGenericValueImpl<T, typename Void<typename T::EncodingType>::Type,
	...
	: IsBaseOf<GenericValue<typename T::EncodingType,
...
class GenericValue {
	...
	typedef typename Encoding::Ch
		Ch; //!< Character type derived from Encoding.
	typedef GenericStringRef<Ch>
		StringRefType; //!< Reference to a constant string
	typedef typename GenericMemberIterator<false, Encoding,
					       Allocator>::Iterator
		MemberIterator; //!< Member iterator for iterating in object.
	typedef typename GenericMemberIterator<true, Encoding,
					       Allocator>::Iterator
  ...
  */
	explicit GenericValue(Type type) RAPIDJSON_NOEXCEPT : data_(), flags_()
	{
		static const unsigned defaultFlags[7] = {
			kNullFlag,     kFalseFlag, kTrueFlag,
		  ...
		  flags_(kNumberInt64Flag)
	{
		...
		if (i64 >= 0) {
			...
			if (!(static_cast<uint64_t>(i64) &
				...
				flags_ |= kIntFlag;
		} else if (i64 >= static_cast<int64_t>(RAPIDJSON_UINT64_C2(
  ...
  */
	~GenericValue()
	{
		if (Allocator::kNeedFree) { // Shortcut by Allocator's trait
			...
			case kObjectFlag:
				for (MemberIterator m = MemberBegin();
				     m != MemberEnd(); ++m)
					m->~Member();
  ...
  */
   ...
   */
	template <typename SourceAllocator>
	GenericValue &
	CopyFrom(const GenericValue<Encoding, SourceAllocator> &rhs,
		 Allocator &allocator)
	{
  ...
  */
	template <typename SourceAllocator>
	bool
	operator==(const GenericValue<Encoding, SourceAllocator> &rhs) const
	{
		...
		case kObjectType: // Warning: O(n^2) inner-loop
			...
			for (ConstMemberIterator lhsMemberItr = MemberBegin();
				...
				typename RhsType::ConstMemberIterator
					rhsMemberItr = rhs.FindMember(
						lhsMemberItr->name);
  ...
  */
	...
	RAPIDJSON_DISABLEIF_RETURN(
		(internal::OrExpr<internal::IsPointer<T>,
  ...
  */
	template <typename T>
	RAPIDJSON_DISABLEIF_RETURN(
		(internal::NotExpr<internal::IsSame<
  ...
  */
	template <typename SourceAllocator>
	bool
	HasMember(const GenericValue<Encoding, SourceAllocator> &name) const
	{
		return FindMember(name) != MemberEnd();
	}
				...
				Allocator &allocator)
	{
		...
		if (o.size >= o.capacity) {
			if (o.capacity == 0) {
				...
				o.members = reinterpret_cast<Member *>(
					allocator.Malloc(o.capacity *
							 sizeof(Member)));
			} else {
				SizeType oldCapacity = o.capacity;
				o.capacity += (oldCapacity + 1) /
					      2; // grow by factor 1.5
				o.members = reinterpret_cast<Member *>(
					allocator.Realloc(
						o.members,
  ...
  */
	...
	{
		return (*this).template PushBack<StringRefType>(value,
								allocator);
	}
	...
	double GetDouble() const
	{
		...
		if ((flags_ & kInt64Flag) != 0)
			return (double)data_.n
				.i64; // int64_t -> double (may lose precision)
  ...
  */
	...
	{
		RAPIDJSON_ASSERT(IsString());
		return ((flags_ & kInlineStrFlag) ? (data_.ss.GetLength()) :
						    data_.s.length);
	}
  ...
  */
	template <typename Handler> bool Accept(Handler &handler) const
	{
		...
		case kObjectType:
			if (!handler.StartObject())
				return false;
			for (ConstMemberIterator m = MemberBegin();
			     m != MemberEnd(); ++m) {
				RAPIDJSON_ASSERT(
					m->name.IsString()); // User may change the type of name by MemberIterator.
				if (!handler.Key(m->name.GetString(),
	...
	enum {
		...
		kNumberAnyFlag = kNumberType | kNumberFlag | kIntFlag |
				 kInt64Flag | kUintFlag | kUint64Flag |
				 kDoubleFlag,
		...
		kTypeMask =
			0xFF // bitwise-and with mask of 0xFF can be optimized by compiler
	};
			 ...
			 Allocator &allocator)
	{
		flags_ = kArrayFlag;
		if (count) {
			data_.a.elements = (GenericValue *)allocator.Malloc(
				count * sizeof(GenericValue));
			std::memcpy(data_.a.elements, values,
				    count * sizeof(GenericValue));
		} else
	...
	void SetObjectRaw(Member *members, SizeType count, Allocator &allocator)
	{
		flags_ = kObjectFlag;
		if (count) {
			data_.o.members = (Member *)allocator.Malloc(
				count * sizeof(Member));
			std::memcpy(data_.o.members, members,
				    count * sizeof(Member));
		} else
	...
	void SetStringRaw(StringRefType s, Allocator &allocator)
	{
		...
		if (ShortString::Usable(s.length)) {
			...
			data_.s.length = s.length;
			str = (Ch *)allocator.Malloc((s.length + 1) *
						     sizeof(Ch));
			data_.s.str = str;
		}
	...
	template <typename SourceAllocator>
	bool
	StringEqual(const GenericValue<Encoding, SourceAllocator> &rhs) const
	{
		RAPIDJSON_ASSERT(IsString());
...
class GenericDocument : public GenericValue<Encoding, Allocator> {
  ...
  */
	...
	GenericDocument &ParseStream(InputStream &is)
	{
		return ParseStream<kParseDefaultFlags, Encoding, InputStream>(
			is);
	}
	//!@}
  ...
  */
	...
	bool EndObject(SizeType memberCount)
	{
		typename ValueType::Member *members =
			stack_.template Pop<typename ValueType::Member>(
				memberCount);
		stack_.template Top<ValueType>()->SetObjectRaw(
			members, (SizeType)memberCount, GetAllocator());
		return true;
	}
