...
#include "document.h"
#include "util/rapidjson/internal/itoa.h"
...
class GenericPointer {
  ...
  */
	explicit GenericPointer(const Ch *source, Allocator *allocator = 0)
		: allocator_(allocator), ownAllocator_(), nameBuffer_(),
  ...
  */
	GenericPointer(const Ch *source, size_t length,
		       Allocator *allocator = 0)
		: allocator_(allocator), ownAllocator_(), nameBuffer_(),
		  tokens_(), tokenCount_(), parseErrorOffset_(),
		  parseErrorCode_(kPointerParseErrorNone)
	...
	GenericPointer &operator=(const GenericPointer &rhs)
	{
		if (this != &rhs) {
			...
			if (rhs.nameBuffer_)
				...
				tokens_ =
					rhs.tokens_; // User supplied const tokens.
				nameBuffer_ = 0;
			}
		}
		return *this;
  ...
  */
	...
	{
		char buffer[21];
		SizeType length = (sizeof(SizeType) == 4 ?
					   internal::u32toa(index, buffer) :
  ...
  */
	...
	{
		if (token.IsString())
			return Append(token.GetString(),
				      token.GetStringLength(), allocator);
		else {
			RAPIDJSON_ASSERT(token.IsUint64());
			RAPIDJSON_ASSERT(token.GetUint64() <= SizeType(~0));
			return Append(static_cast<SizeType>(token.GetUint64()),
				      allocator);
		}
	}
  ...
  */
			  ...
			  bool *alreadyExist = 0) const
	{
		...
		ValueType *v = &root;
		bool exist = true;
		for (const Token *t = tokens_; t != tokens_ + tokenCount_;
		     ++t) {
			if (v->IsArray() && t->name[0] == '-' &&
			    t->length == 1) {
				...
				if (t->index ==
				    kPointerInvalidIndex) { // must be object name
					if (!v->IsObject())
  ...
  */
	ValueType *Get(ValueType &root) const
	{
		...
		for (const Token *t = tokens_; t != tokens_ + tokenCount_;
			...
			case kObjectType: {
				typename ValueType::MemberIterator m =
					v->FindMember(GenericStringRef<Ch>(
						t->name, t->length));
				if (m == v->MemberEnd())
					return 0;
				v = &m->value;
  ...
  */
	...
	GetWithDefault(ValueType &root, T defaultValue,
		       typename ValueType::AllocatorType &allocator) const
	{
		return GetWithDefault(root, ValueType(defaultValue).Move(),
				      allocator);
	}
	...
	{
		return GetWithDefault(document, defaultValue,
  ...
  */
	...
	ValueType &Set(ValueType &root, const Ch *value,
		       typename ValueType::AllocatorType &allocator) const
	{
		return Create(root, allocator) =
			       ValueType(value, allocator).Move();
	}
  ...
  */
	...
	{
		return Create(document).CopyFrom(value,
						 document.GetAllocator());
	}
  ...
  */
	bool Erase(ValueType &root) const
	{
		...
		case kArrayType:
			if (last->index == kPointerInvalidIndex ||
			    last->index >= v->Size())
				return false;
  ...
  */
			...
			size_t extraNameBufferSize = 0)
	{
		...
		size_t nameBufferSize =
			rhs.tokenCount_; // null terminators for tokens
		for (Token *t = rhs.tokens_; t != rhs.tokens_ + rhs.tokenCount_;
		     ++t)
		...
		std::memcpy(tokens_, rhs.tokens_,
			    rhs.tokenCount_ * sizeof(Token));
  ...
  */
	void Parse(const Ch *source, size_t length)
	{
		...
		Token *token = tokens_ = static_cast<Token *>(
			allocator_->Malloc(tokenCount_ * sizeof(Token) +
					   length * sizeof(Ch)));
		Ch *name = nameBuffer_ =
			reinterpret_cast<Ch *>(tokens_ + tokenCount_);
		size_t i = 0;
		...
		RAPIDJSON_ASSERT(name <=
				 nameBuffer_ +
					 length); // Should not overflow buffer
		parseErrorCode_ = kPointerParseErrorNone;
		return;
		...
		for (Token *t = tokens_; t != tokens_ + tokenCount_; ++t) {
			...
			for (size_t j = 0; j < t->length; j++) {
				...
				if (c == '~') {
					...
					GenericStringStream<
						typename ValueType::EncodingType>
						source(&t->name[j]);
					PercentEncodeStream<OutputStream> target(
						os);
					if (!Transcoder<EncodingType, UTF8<> >()
