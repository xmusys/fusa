...
class PrettyWriter : public Writer<OutputStream, SourceEncoding, TargetEncoding,
  ...
  */
	...
	bool EndObject(SizeType memberCount = 0)
	{
		...
		RAPIDJSON_ASSERT(!Base::level_stack_
					  .template Top<typename Base::Level>()
					  ->inArray);
		bool empty = Base::level_stack_
				     .template Pop<typename Base::Level>(1)
				     ->valueCount == 0;
	...
	void WriteIndent()
	{
		size_t count = (Base::level_stack_.GetSize() /
				sizeof(typename Base::Level)) *
			       indentCharCount_;
