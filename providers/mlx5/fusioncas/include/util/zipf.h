			     ...
			     double theta)
{
	...
	while (last_n < n) {
		last_sum +=
			1. / mehcached_pow_approx((double)last_n + 1., theta);
		last_n++;
	}
	return last_sum;
}
...
static uint64_t mehcached_zipf_next(struct zipf_gen_state *state)
{
	if (state->last_n != state->n) {
		if (state->theta > 0. && state->theta < 1.) {
			state->zetan =
				mehcached_zeta(state->last_n, state->zetan,
					       state->n, state->theta);
			state->eta =
				(1. -
				 mehcached_pow_approx(2. / (double)state->n,
						      1. - state->theta)) /
				(1. - mehcached_zeta(0, 0., 2, state->theta) /
					      state->zetan);
		}
