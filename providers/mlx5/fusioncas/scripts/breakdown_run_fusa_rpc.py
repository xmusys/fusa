...
agent_exec_file = "fusa_rpc_agent"
...
def start_agent_server(threshold, agent_server_log, exp_type):
    ...
    try:
        cmd = f"../build/agent/{agent_exec_file} server {threshold} {exp_type}"
...
def start_agent_client(threshold, log_file, client_ip, client_port, client_user, client_pass):
    ...
    try:
        ...
        if client_ip == "192.168.6.9":
            command = f"cd /home/sqs/rdma-core/providers/mlx5/fusioncas/build && nohup ./agent/{agent_exec_file} client > {remote_log} 2>&1 &"
        else:
            command = f"cd /home/dell/sqs/rdma-core/providers/mlx5/fusioncas/build && nohup ./agent/{agent_exec_file} client > {remote_log} 2>&1 &"
                ...
                for thread_num in THREAD_NUMS:
                    for depth in DEPTHS:
                        for threshold in THRESHOLDS:
                            ...
                            agent_server_log = f"../data/agent/{exp_prefix}/latency_{exp_type}_agent_server_{distribution_type}_{ycsb_type}_{thread_num}_{depth}_{threshold}_{cur_time}.log"
...
time.sleep(10) # retry after 10s if startup fails
                            ...
                            for i in range(len(CLIENT_IPS)):
                                ...
                                log_file = f"../data/{CLIENT_IPS[i]}/{exp_prefix}/latency_{exp_type}_{distribution_type}_{ycsb_type}_{CLIENT_IPS[i]}_{thread_num}_{depth}_{threshold}_client{i+1}_{cur_time}.log"
