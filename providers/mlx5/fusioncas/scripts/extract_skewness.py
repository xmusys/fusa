...
# , support ycsb_type and timestamp
    pattern = r"latency_([^_]+)_([^_]+)_([\w][\w._]+[\w])_(\d+\.\d+\.\d+\.\d+)_(\d+)_(\d+)_(\d+)_client(\d+)_(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\.\d+)\.log"
    match = re.match(pattern, os.path.basename(filename))
    if match:
        return {
            "exp_t": match.group(1),          # enable
            "dist_t": match.group(2),         # zipfian
"ycsb_type": match.group(3), # ycsb_a0.9 ycsb_a0_9
    ...
    try:
        with open(log_file, 'r', encoding='utf-8') as f:
            ...
            if attempts_match:
                ...
                metrics["IOPS"] = metrics["attempts"] / (times * 1000000)
