...
SERVER_COMMAND = '../build/test/test_ycsb server'  # server program path
...
def start_server_rdma():
    logging.info("start server-side  ./test/test_ycsb...")
    try:
        ...
        logging.error(f"start server-side  ./test/test_ycsb : failed:  {e}")
        return None
...
def start_client_rdma(thread_num, depth, stride_value, log_file, client_ip, client_port, client_user, client_pass):
    logging.info(f"via  SSH start client-side  ./test/test_ycsb {thread_num} {depth} {stride_value}...")
    ...
    # # execute command: cd to dir, run  test_ycsb
    if client_ip == "192.168.6.9":
        command = f'cd /home/sqs/rdma-core/providers/mlx5/fusioncas/build && LD_PRELOAD="./libmlx5.so ./libibverbs.so" ./test/test_ycsb client {thread_num} {depth}'
    else:
        command = f'cd /home/dell/sqs/rdma-core/providers/mlx5/fusioncas/build && LD_PRELOAD="./libmlx5.so ./libibverbs.so" ./test/test_ycsb client {thread_num} {depth}'
...
def kill_server_rdma():
    logging.info("stop server-side  ./test/test_ycsb...\n\n")
    try:
        # force kill via pkill test_rdma  process
        subprocess.run("pkill -9 test_ycsb", shell=True, check=True)
        logging.info("terminated  test_ycsb  process.")
    except subprocess.CalledProcessError as e:
        logging.error(f"kill  test_ycsb  process: failed:  {e}")
        ...
        for thread_num in THREAD_NUMS:
            for depth in DEPTHS:
                for threshold in THRESHOLDS:
                    ...
                    agent_server_log = f"../data/agent/agent_server_{ycsb_name}_{thread_num}_{depth}_{threshold}.log"
                    agent_server = start_agent_server(threshold, agent_server_log)
                    
                    # 2. start all agent clients
                    agent_clients = []
                    for i in range(len(CLIENT_IPS)):
                        log_file = f"../data/agent/agent_client_{ycsb_name}_{CLIENT_IPS[i]}_{thread_num}_{depth}_{threshold}.log"
                    ...
                    # start server -side  test_ycsb,  retry until success
                    server_process = None
                    retries = 0
...
time.sleep(10) # retry after 10s if startup fails
                    ...
                    if server_process is None:
                        logging.error("could not start server-side   test_ycsb, exit")
                    ...
                    for i in range(len(CLIENT_IPS)):
                        ...
                        logging.info(f"start  {CLIENT_IPS[i]} -side  test_ycsb, threads: {thread_num}, depth: {depth}...")

                        # create log filename, format:  thread_num_depth_stride_clientX.log
                        log_file = f"../data/{CLIENT_IPS[i]}/{ycsb_name}_{CLIENT_IPS[i]}_{thread_num}_{depth}_{threshold}_client{i+1}.log"
