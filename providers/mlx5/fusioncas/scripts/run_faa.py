...
logging.info(f"start {client_ip} client ./test/{execute_file}, threads: {thread_num}, depth: {depth}")
...
#
    if client_ip == "192.168.6.9":
        command = f'cd /home/sqs/rdma-core/providers/mlx5/fusioncas/build && make -j && ./test/{execute_file} client {thread_num} {depth}'
    else:
        command = f'cd /home/dell/sqs/rdma-core/providers/mlx5/fusioncas/build && make -j && ./test/{execute_file} client {thread_num} {depth}'
...
ssh_clients = [] # SSH client
                            ...
                            for i in range(len(CLIENT_IPS)):
                                ...
                                client_pass = CLIENT_PASSWORDS[i]
                                if variant == "":
                                    log_file = f"../data/{client_ip}/{exp_prefix}/latency_{exp_type}_{distribution_type}_{ycsb_type}_{client_ip}_{thread_num}_{depth}_{threshold}_client{i+1}_{log_time}.log"
                                else:
                                    log_file = f"../data/{client_ip}/{exp_prefix}/latency_{variant}_{exp_type}_{distribution_type}_{ycsb_type}_{client_ip}_{thread_num}_{depth}_{threshold}_client{i+1}_{log_time}.log"

                                thread = threading.Thread(target=start_client_rdma, 
                                                    args=(thread_num, depth, 0, log_file, client_ip, client_port, client_user, client_pass, ssh_clients))
