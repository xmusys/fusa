...
execute_file = "faa_stride"
SERVER_COMMAND = f'../build/test/{execute_file} server'
THREAD_NUMS = [32]
THRESHOLDS = [0]
STRIDES = [16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192]
DEPTHS = [1]
...
YCSB_TYPES = ["update"]
# YCSB_TYPES = ["u60r40", "ycsb_a", "u40r60", "u30r70", "u20r80", "u10r90"]
exp_type = "disable"
variant = "stride"
exp_prefix = "faa_exp"
DIST_TYPES = ["uniform"]
...
# start client-side  test_herd, execute via SSH
...
logging.info(f"start {client_ip} client ./test/{execute_file}, threads: {thread_num}, depth: {depth} : {stride_value}")
    ...
    ssh_client.connect(client_ip, username=client_user, password=client_pass, port=client_port)
    ssh_client_ref.append(ssh_client)  # keep SSH client reference

#
    if client_ip == "192.168.6.9":
        command = f'cd /home/sqs/rdma-core/providers/mlx5/fusioncas/build && make -j && ./test/{execute_file} client {thread_num} {depth} {stride_value}'
    else:
        command = f'cd /home/dell/sqs/rdma-core/providers/mlx5/fusioncas/build && make -j && ./test/{execute_file} client {thread_num} {depth} {stride_value}'
...
logging.warning("server processkill , ")
        subprocess.run(f"pkill -9 {execute_file}", shell=True)
...
def modify_herd_config(config_path: str, 
                    new_path: str,
    ...
    try:
        ...
        for i in range(len(CLIENT_IPS)):
            ...
            if CLIENT_IPS[i] == "192.168.6.9":
                remote_path_ = f"/home/sqs/rdma-core/providers/mlx5/fusioncas/config/herd.json"
            else:
                remote_path_ = f"/home/dell/sqs/rdma-core/providers/mlx5/fusioncas/config/herd.json"
            scp_config(
...
def main():
    for distribution_type in DIST_TYPES:
        ...
        for ycsb_type in YCSB_TYPES:
            ...
            modify_herd_config("../config/herd.json", ycsb_path, False)
                            ...
                            while retry_count < max_retries:
                                ...
                                while server_process is None and server_retries < 3:
                                    ...
                                    if server_process is None:
                                        logging.error(f"startup failed, wait  10 s before retry... (attempt  {server_retries + 1})")
                                        time.sleep(10)
                                        server_retries += 1
...
ssh_clients = [] # SSH client
                                ...
                                for i in range(len(CLIENT_IPS)):
                                    ...
                                    client_pass = CLIENT_PASSWORDS[i]
                                    if variant == "":
                                        log_file = f"../data/{client_ip}/{exp_prefix}/latency_{exp_type}_{distribution_type}_{ycsb_type}_{client_ip}_{thread_num}_{depth}_{stride}_{threshold}_client{i+1}_{log_time}.log"
                                    else:
                                        log_file = f"../data/{client_ip}/{exp_prefix}/latency_{variant}_{exp_type}_{distribution_type}_{ycsb_type}_{client_ip}_{thread_num}_{depth}_{stride}_{threshold}_client{i+1}_{log_time}.log"

                                    thread = threading.Thread(target=start_client_rdma, 
                                                        args=(thread_num, depth, stride, log_file, client_ip, client_port, client_user, client_pass, ssh_clients))
                                ...
                                # all threads completed on time, stop server and break loop
