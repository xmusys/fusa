...
THRESHOLDS = [50000, 80000]#] 15000, 20000, 25000, 30000, 35000, 40000, 45000, 50000]          # client threshold
...
YCSB_TYPES = ["ycsb_a"] #, "u70r30", "ycsb_a", "u30r70"] #["u90r10", "u80r20","u70r30", "u60r40", 
...
def start_client_rdma(thread_num, depth, stride_value, log_file, client_ip, client_port, client_user, client_pass, ssh_client_ref):
    ...
    ssh_client_ref.append(ssh_client)  # keep SSH client reference
...
def kill_server_rdma(server_process):
...
def start_agent_server(threshold, agent_server_log, exp_type, core_num):
    # """start local agent server"""
    logging.info(f"start agent server (threshold={threshold})...")
    try:
        cmd = f"../build/agent/test_agent server {threshold} {exp_type} disable {core_num}"
        server_proc = subprocess.Popen(cmd.split(), 
                                      stdout=open(agent_server_log, 'w'),
                                      stderr=subprocess.STDOUT)
        time.sleep(5)  # wait for server init
        return server_proc
    except Exception as e:
        logging.error(f"start agent server : failed:  {str(e)}")
        return None

# client-side test_herd process
def kill_all_clients(ssh_clients):
    logging.info(f"kill all client-side  ./test/{execute_file}...")
    for ssh_client in ssh_clients:
        try:
            ssh_client.exec_command(f"pkill -9 {execute_file}")
            ssh_client.close()
            logging.info(f"terminated  {ssh_client.get_transport().getpeername()[0]}   {execute_file}  process")
        except Exception as e:
            logging.error(f"kill client  process: failed:  {e}")
...
def modify_server_config(config_path: str,
                    core_num: int,
    ...
    try:
        ...
        for i in range(len(CLIENT_IPS)):
            with open(config_path, 'r+', encoding='utf-8') as f:
                ...
                if 'memory_node' not in config:
                    raise KeyError("JSON missing memory_node field")
                    
                memory_node = config['memory_node']
                if not isinstance(memory_node, dict):
                    raise TypeError("memory_node type error, expected dict")
                
                # modify target field
                memory_node['rpc_param']['rpc_thread_num'] = core_num
...
def modify_app_config(config_path: str, 
                    new_path: str,
                    backup: bool = True) -> Union[bool, Exception]:
    # """
    ...
    try:
        ...
        for i in range(len(CLIENT_IPS)):
            ...
            if CLIENT_IPS[i] == "192.168.6.9":
                remote_path_ = f"/home/sqs/rdma-core/providers/mlx5/fusioncas/config/{config_name}"
            else:
                remote_path_ = f"/home/dell/sqs/rdma-core/providers/mlx5/fusioncas/config/{config_name}"
            scp_config(
        ...
        for exp_type in EXP_TYPES:
            for ycsb_type in YCSB_TYPES:
                for core_num in CORES:
                    modify_server_config(f"../config/{config_name}", core_num, False)
                    ycsb_path = ycsb_prefix + ycsb_type
                    modify_app_config(f"../config/{config_name}", ycsb_path, False)
                    modify_agent_config("../config/agent.json", " ", False)

                    for thread_num in THREAD_NUMS:
                        for depth in DEPTHS:
                            for threshold in THRESHOLDS:
                                max_retries = 5  # allow retries  5 
                                retry_count = 0
                                ...
                                while retry_count < max_retries:
                                    ...
                                    logging.info(f"starting execution: : {exp_prefix}/latency_{exp_type}_{distribution_type}_{ycsb_type}_{thread_num}_{depth}_{threshold}_{core_num}")

                                    # 1. start agent server
                                    agent_server_log = f"../exp_log/agent/{exp_prefix}/latency_{exp_type}_agent_server_{distribution_type}_{ycsb_type}_{thread_num}_{depth}_{threshold}_{core_num}_{cur_time}.log"
                                    agent_server = start_agent_server(threshold, agent_server_log, exp_type, core_num)
                                    
                                    # 2. start all agent clients
                                    for i in range(len(CLIENT_IPS)):
                                        log_file = f"../exp_log/agent/{exp_prefix}/latency_{exp_type}_agent_client_{distribution_type}_{ycsb_type}_{CLIENT_IPS[i]}_{thread_num}_{depth}_{threshold}_{core_num}.log"
                                        start_agent_client(threshold, log_file, CLIENT_IPS[i], CLIENT_PORTS[i], CLIENT_USERS[i], CLIENT_PASSWORDS[i])
                                    ...
                                    server_process = None
                                    server_retries = 0
                                    while server_process is None and server_retries < 3:
                                        server_process = start_server_rdma()
                                        if server_process is None:
logging.error(f"server startup failed, wait 10 s retry... ( {server_retries + 1} )")
                                            time.sleep(10)
                                            server_retries += 1
                                    ...
                                    # 4. start client threads
                                    threads = []
                                    ssh_clients = []  # track all SSH client connections
                                    ...
                                    for i in range(len(CLIENT_IPS)):
                                        ...
                                        client_user = CLIENT_USERS[i]
                                        client_pass = CLIENT_PASSWORDS[i]
                                        log_file = f"../exp_log/{CLIENT_IPS[i]}/{exp_prefix}/latency_{exp_type}_{distribution_type}_{ycsb_type}_{CLIENT_IPS[i]}_{thread_num}_{depth}_{threshold}_{core_num}_client{i+1}_{cur_time}.log"

                                        thread = threading.Thread(target=start_client_rdma, args=(thread_num, depth, 0, log_file, client_ip, client_port, client_user, client_pass, ssh_clients))
                                        threads.append(thread)
                                        thread.start()

                                    # 5. wait all clients done or timeout
                                    # wait all threads done or timeout
                                    timeout = 600  # 10 min
                                    all_finished = True
                                    for thread in threads:
                                        thread.join(timeout=(timeout - (time.time() - start_time) if time.time() - start_time < timeout else 0.1))
                                        if thread.is_alive():
                                            all_finished = False
                                            break

                                    elapsed_time = time.time() - start_time
                                    if not all_finished or elapsed_time > timeout:
                                        logging.warning(f"experiment exceeded  {timeout} s, kill all processes, retry (attempt  {retry_count + 1})")
                                        kill_all_clients(ssh_clients)
                                        kill_server_rdma(server_process)
                                        stop_agent_clients()
                                        # kill  agent server
                                        if agent_server:
                                            agent_server.terminate()
                                        else:
                                            logging.warning("agent server process not found")
                                        time.sleep(10)  # wait for cleanup
                                        retry_count += 1
                                        continue

                                    # 6. all threads completed on time, stop server and break loop
                                    logging.info(f"all clients done, took  {elapsed_time:.2f} s")
                                    kill_server_rdma(server_process)
                                    stop_agent_clients()
                                    time.sleep(15)
                                    ...
                                    break  # experiment succeeded, break retry loop

                                if retry_count >= max_retries:
                                    logging.error(f"experiment abandoned after  {max_retries}  retries still timed out, skipping")
