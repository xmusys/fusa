...
YCSB_TYPES = ["update", "ycsb_a","u30r70"] #, "u70r30", "ycsb_a", "u30r70"] #["u90r10", "u80r20","u70r30", "u60r40", 
                        ...
                        for depth in DEPTHS:
                            for threshold in THRESHOLDS:
                                ...
                                while retry_count < max_retries:
                                    ...
                                    logging.info(f"starting execution: : {exp_prefix}/latency_{exp_type}_{distribution_type}_{ycsb_type}_{thread_num}_{depth}_{threshold}_{core_num}_{pcie}")

                                    # 1. start agent server
                                    agent_server_log = f"../exp_log/agent/{exp_prefix}/latency_{exp_type}_agent_server_{distribution_type}_{ycsb_type}_{thread_num}_{depth}_{threshold}_{core_num}_{pcie}_{cur_time}.log"
                                    agent_server = start_agent_server(threshold, agent_server_log, exp_type)
                                    
                                    # 2. start all agent clients
                                    for i in range(len(CLIENT_IPS)):
                                        log_file = f"../exp_log/agent/{exp_prefix}/latency_{exp_type}_agent_client_{distribution_type}_{ycsb_type}_{CLIENT_IPS[i]}_{thread_num}_{depth}_{threshold}_{core_num}_{pcie}.log"
...
logging.error(f"server startup failed, wait 10 s retry... ( {server_retries + 1} )")
                                    ...
                                    for i in range(len(CLIENT_IPS)):
                                        ...
                                        client_user = CLIENT_USERS[i]
                                        client_pass = CLIENT_PASSWORDS[i]
                                        log_file = f"../exp_log/{CLIENT_IPS[i]}/{exp_prefix}/latency_{exp_type}_{distribution_type}_{ycsb_type}_{CLIENT_IPS[i]}_{thread_num}_{depth}_{threshold}_{core_num}_client{i+1}_{pcie}_{cur_time}.log"
