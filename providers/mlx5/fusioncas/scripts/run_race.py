...
execute_file = "test_ht"
client_execute_file = "hashtable_bench"
SERVER_COMMAND = f'../build/test/{execute_file} server'  # server program path
# SERVER_COMMAND = './test/test_rdma'
THREAD_NUMS = [8]#, 8, 14, 10, 6, 26, 22, 18]#, 24, 20, 16, 12, 8, 4]#[28, 24, 20, 16, 12, 8] #, 28, 24, 20, 16, 12, 8]  # client thread count
THRESHOLDS = [80000, 999999]#] 15000, 20000, 25000, 30000, 35000, 40000, 45000, 50000]          # client threshold
...
YCSB_TYPES = ["ycsb_a50m"]#["update","u70r30", "ycsb_a", "u30r70"] #, "u70r30", "ycsb_a", "u30r70"] #["u90r10", "u80r20","u70r30", "u60r40", 
...
DIST_TYPES = ["zipfian"]
config_name = "datastructure.json"
...
def start_client_rdma(thread_num, depth, stride_value, log_file, client_ip, client_port, client_user, client_pass, ssh_client_ref):
    ...
    if client_ip == "192.168.6.9":
        command = f'cd /home/sqs/rdma-core/providers/mlx5/fusioncas/build && make -j && LD_PRELOAD="./libmlx5.so ./libibverbs.so" ./smart_ht/{client_execute_file} {thread_num} {depth}'
    else:
        command = f'cd /home/dell/sqs/rdma-core && ./make.sh && cd /home/dell/sqs/rdma-core/providers/mlx5/fusioncas/build && make -j && LD_PRELOAD="./libmlx5.so ./libibverbs.so" ./smart_ht/{client_execute_file} {thread_num} {depth}'
...
def kill_all_clients(ssh_clients):
    logging.info(f"kill all client-side  ./test/{client_execute_file}...")
    for ssh_client in ssh_clients:
        try:
            ssh_client.exec_command(f"pkill -9 {client_execute_file}")
            ssh_client.close()
            logging.info(f"terminated  {ssh_client.get_transport().getpeername()[0]}   {client_execute_file}  process")
        except Exception as e:
    ...
    try:
        ...
        for i in range(len(CLIENT_IPS)):
            with open(config_path, 'r+', encoding='utf-8') as f:
                ...
                # modify target field
                config['ycsb_path'] = new_path
                config['machine_id'] = i
                if i == 0:
                    config['insert_before_execution'] = True
                else:
                    config['insert_before_execution'] = False
                    ...
                    for depth in DEPTHS:
                        for threshold in THRESHOLDS:
                            ...
                            while retry_count < max_retries:
                                ...
                                agent_server_log = f"../exp_log/agent/{exp_prefix}/latency_{exp_type}_agent_server_{distribution_type}_{ycsb_type}_{thread_num}_{depth}_{threshold}_{cur_time}.log"
                                agent_server = start_agent_server(threshold, agent_server_log, exp_type)
                                
                                # 2. start all agent clients
                                for i in range(len(CLIENT_IPS)):
                                    log_file = f"../exp_log/agent/{exp_prefix}/latency_{exp_type}_agent_client_{distribution_type}_{ycsb_type}_{CLIENT_IPS[i]}_{thread_num}_{depth}_{threshold}.log"
...
logging.error(f"server startup failed, wait 10 s retry... ( {server_retries + 1} )")
                                ...
                                for i in range(len(CLIENT_IPS)):
                                    ...
                                    client_user = CLIENT_USERS[i]
                                    client_pass = CLIENT_PASSWORDS[i]
                                    log_file = f"../exp_log/{CLIENT_IPS[i]}/{exp_prefix}/latency_{exp_type}_{distribution_type}_{ycsb_type}_{CLIENT_IPS[i]}_{thread_num}_{depth}_{threshold}_client{i+1}_{cur_time}.log"
