...
THRESHOLDS = [50000, 80000, 10000000]#] 15000, 20000, 25000, 30000, 35000, 40000, 45000, 50000]          # client threshold
...
YCSB_TYPES = ["update0.9", "update0.5", "update0.3"] #, "u70r30", "ycsb_a", "u30r70"] #["u90r10", "u80r20","u70r30", "u60r40", 
...
EXP_TYPES = ["enable"]
exp_prefix = "pcie_atomic/skewness"
DIST_TYPES = ["zipfian"]
...
def start_agent_server(threshold, agent_server_log, exp_type):
    ...
    try:
        cmd = f"../build/agent/test_agent server {threshold} {exp_type} {pcie_enable}"
...
def main():
    for distribution_type in DIST_TYPES:
        ycsb_prefix = f"/home/dell/sqs/ycsb_output/1-node/{distribution_type}/{sub_dir}/"
                    ...
                    for depth in DEPTHS:
                        for threshold in THRESHOLDS:
                            ...
                            while retry_count < max_retries:
                                ...
                                logging.info(f"starting execution: : {exp_prefix}/latency_{exp_type}_{distribution_type}_{ycsb_type}_{thread_num}_{depth}_{threshold}_{pcie}")

                                # 1. start agent server
                                agent_server_log = f"../exp_log/agent/{exp_prefix}/latency_{exp_type}_agent_server_{distribution_type}_{ycsb_type}_{thread_num}_{depth}_{threshold}_{pcie}_{cur_time}.log"
                                agent_server = start_agent_server(threshold, agent_server_log, exp_type)
                                
                                # 2. start all agent clients
                                for i in range(len(CLIENT_IPS)):
                                    log_file = f"../exp_log/agent/{exp_prefix}/latency_{exp_type}_agent_client_{distribution_type}_{ycsb_type}_{CLIENT_IPS[i]}_{thread_num}_{depth}_{pcie}_{threshold}.log"
...
logging.error(f"server startup failed, wait 10 s retry... ( {server_retries + 1} )")
                                ...
                                for i in range(len(CLIENT_IPS)):
                                    ...
                                    client_user = CLIENT_USERS[i]
                                    client_pass = CLIENT_PASSWORDS[i]
                                    log_file = f"../exp_log/{CLIENT_IPS[i]}/{exp_prefix}/latency_{exp_type}_{distribution_type}_{ycsb_type}_{CLIENT_IPS[i]}_{thread_num}_{depth}_{threshold}_client{i+1}_{pcie}_{cur_time}.log"
