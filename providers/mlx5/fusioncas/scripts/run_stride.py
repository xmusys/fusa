...
SERVER_COMMAND = '../build/test/test_latency server'  # server program path
# SERVER_COMMAND = './test/test_rdma'
THREAD_NUMS = [32]  # client thread count
THRESHOLDS = [10000, 15000, 20000]#, 25000, 30000, 35000, 40000, 45000, 50000]          # client threshold
# STRIDES = [512, 1024, 2048, 4096, 8192, 8, 16, 32, 64, 128, 256]
STRIDES = [128, 256]
# DEPTHS = [1, 8, 4, 2, 16]
DEPTHS = [1]
CLIENT_IPS = ["192.168.6.4", "192.168.6.3", "192.168.6.5", "192.168.6.9"]  # client IP address array
CLIENT_PORTS = [2222, 2222, 2222, 22222]  # client port array
CLIENT_USERS = ["dell", "dell", "dell", "sqs"]  # client username array
CLIENT_PASSWORDS = ["204msjcllm", "204msjcllm@1OI.c0M", "204msjcllm", "204msjcllm"]  # client password array
YCSB_PATHS = ["../data/update", "../data/ycsb_a"]
...
def start_server_rdma():
    logging.info("start server-side  ./test/test_latency...")
    try:
        ...
        logging.error(f"start server-side  ./test/test_latency : failed:  {e}")
        return None
...
def start_client_rdma(thread_num, depth, stride_value, log_file, client_ip, client_port, client_user, client_pass):
    logging.info(f"via  SSH start client-side  ./test/test_latency {thread_num} {depth} {stride_value}...")
    ...
    # # execute command: cd to dir, run  test_latency
    if client_ip == "192.168.6.9":
        command = f'cd /home/sqs/rdma-core/providers/mlx5/fusioncas/build && LD_PRELOAD="./libmlx5.so ./libibverbs.so" ./test/test_latency client {thread_num} {depth} {stride_value}'
    else:
        command = f'cd /home/dell/sqs/rdma-core/providers/mlx5/fusioncas/build && LD_PRELOAD="./libmlx5.so ./libibverbs.so" ./test/test_latency client {thread_num} {depth} {stride_value}'


    # create log file on client, redirect program output
...
def kill_server_rdma():
    logging.info("stop server-side  ./test/test_latency...\n\n")
    try:
        # force kill via pkill test_rdma  process
        subprocess.run("pkill -9 test_latency", shell=True, check=True)
        logging.info("terminated  test_stride  process.")
    except subprocess.CalledProcessError as e:
        logging.error(f"kill  test_latency  process: failed:  {e}")

def start_agent_server(threshold, agent_server_log):
    # """start local agent server"""
    logging.info(f"start agent server (threshold={threshold})...")
    try:
        cmd = f"../build/agent/test_agent server {threshold}"
...
def scp_config(client_ip, client_port, user, password):
    ...
    try:
        ...
        with SCPClient(ssh.get_transport()) as scp:
            ...
            if client_ip == "192.168.6.9":
                remote_path = f"/home/sqs/rdma-core/providers/mlx5/fusioncas/config/test_rdma.json"
            else:
                remote_path = f"/home/dell/sqs/rdma-core/providers/mlx5/fusioncas/config/test_rdma.json"
            scp.put(CONFIG_PATH, remote_path)
...
def main():
    ...
    for stride in STRIDES:
        # modify config file  function,  can modify more than ycsb path
        ycsb_path  = YCSB_PATHS[0]
        ...
        for thread_num in THREAD_NUMS:
            for depth in DEPTHS:
                for threshold in THRESHOLDS:
                    ...
                    agent_server_log = f"../data/agent/stride_exp/agent_server_stride_{thread_num}_{depth}_{stride}_{threshold}.log"
                    agent_server = start_agent_server(threshold, agent_server_log)
                    
                    # 2. start all agent clients
                    agent_clients = []
                    for i in range(len(CLIENT_IPS)):
                        log_file = f"../data/agent/stride_exp/agent_client_stride_{CLIENT_IPS[i]}_{thread_num}_{depth}_{stride}_{threshold}.log"
                    ...
                    time.sleep(20)  # wait for agent init
                    # iterate over thread and depth arrays, iterate over all combinations

# start server test_stride, retry until success
                    server_process = None
                    retries = 0
...
time.sleep(10) # retry after 10s if startup fails
                    ...
                    if server_process is None:
                        logging.error("could not start server-side   test_stride, exit")
...
logging.info(f"start {CLIENT_IPS[i]} test_stride, threads: {thread_num}, depth: {depth}...")

                        # create log filename, format:  thread_num_depth_stride_clientX.log
                        log_file = f"../data/{CLIENT_IPS[i]}/stride_exp/stride_{CLIENT_IPS[i]}_{thread_num}_{depth}_{stride}_{threshold}_client{i+1}.log"

                        # start client-side  test_rdma,  log output to file
                        thread = threading.Thread(target=start_client_rdma, args=(thread_num, depth, stride, log_file, client_ip, client_port, client_user, client_pass))
