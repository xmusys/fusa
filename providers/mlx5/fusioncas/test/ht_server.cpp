...
void HTServer::init_rpc(JsonConfig &rpc_config)
{
...
int HTServer::start()
{
	...
	// int rc = memory_node.register_main_memory(rpc_addr, mem_pool_size + rpc_region_size);
	int rc = memory_node.register_main_memory(
		rpc_addr, mem_pool_size + rpc_region_size, rpc_region_size,
		true);
	assert(!rc);

	local_addr = rpc_addr + rpc_region_size;

	RemoteHashTable::Setup(memory_node);

	rc = memory_node.start(tcp_port);

	volatile uint64_t *check = (uint64_t *)rpc_addr;
...
void *HTServer::rpc_func(void *rarg)
{
	uint64_t rpc_thread_id = ((thread_arg *)rarg)->thread_id;
	HTServer *server = ((thread_arg *)rarg)->server;
	// std::cout << "bbbbbbbbb" << std::endl;
	server->rpc_func_impl(rpc_thread_id);
}

void *HTServer::rpc_func_impl(uint64_t rpc_thread_id)
{
	std::unordered_map<uint64_t, uint64_t> qpn_table;
	uint64_t tmp_tt_qp = 0;
	// added offset to qpn mapping to prevent collisions when 4 clients restart and qpns all start from 0
	// if a client hangs in mlx5_post_send in the future, this offset may be the issue
						  ...
						  [thread_batch_cnt[cur_idx]]
							...
							(send_tx[cur_idx] &
							 unsignal_batch) == 0 ?
								IBV_SEND_SIGNALED :
								0;
						// changed to send/recv, no resp_addr needed
						wr[cur_idx]
						  [thread_batch_cnt[cur_idx]]
							  .wr.rdma.remote_addr =
							cur_req->resp_addr;
