...
class FAAClient : public BasicClient {
    public:
	uint64_t *keys;
	char *methods;
	...
	// std::unordered_map<uint64_t, uint64_t> tt_hotness;
	uint64_t tt_hotness[__MAX_COUNTER_NUM];
	double tt_slot_latencies[__MAX_COUNTER_NUM];

    public:
	FAAClient(size_t nr_memory_nodes, size_t machine_num, size_t machine_id,
			   ...
			   double *tl_slot_latency)
	{
		...
		if (thread_id == report_thread) {
			...
			SDS_INFO("FAA Clinet report Latencies");
				  ...
				  uint64_t server_id, size_t depth) override
	{
		...
		SDS_INFO(
			...
			thread_id, server_id);
		uint64_t blank[10] = { 40,  10,	 35,  5,   423,
				       388, 428, 458, 398, 363 };
		uint64_t qp_num =
...
#endif
		// std::unordered_map<uint64_t, uint64_t> tl_hotness;
		...
		while (!stop_signal) {
			...
			for (int i = 0; i < depth; i++) {
				if (methods[thread_attepmts_num *
					...
					no_update[i] = false;
					// for(int j = 0; j < 10; j ++) {
					//     if((((remote_va + remote_addr.offset) % 4096 ) / 8) == blank[j]) {
					//         herd_add_request(wr[i], sge[i], i, IBV_WR_RDMA_READ, &read_buf[i].comp, remote_addr, rkey, remote_va, sizeof(uint64_t), 0, 0,
					//         IBV_SEND_SIGNALED, conns[server_id]->manager_, i == (depth - 1) ? NULL : &wr[i + 1]);
					//         no_update[i] = true;
					//         break;
					//     }
					// }
					...
					herd_add_request(
						...
						&recv_buf[i].comp, remote_addr,
						rkey, remote_va, sizeof(rpc_op),
						1, 0, IBV_SEND_SIGNALED,
