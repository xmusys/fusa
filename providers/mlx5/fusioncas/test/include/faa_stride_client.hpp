...
class FAAStrideClient : public BasicClient {
    public:
	uint64_t *keys;
	char *methods;
    ...
    public:
	FAAStrideClient(size_t nr_memory_nodes, size_t machine_num,
				  ...
				  uint64_t server_id, size_t depth) override
	{
		...
		SDS_INFO(
			"FAAStrideClinent thread_test_fun_impl() executed for thread %d and server %d",
			thread_id, server_id);
		uint64_t qp_num =
		...
		while (!stop_signal) {
			...
			for (int i = 0; i < depth; i++) {
				if (methods[thread_attepmts_num *
					...
					// remote_addr.offset = keys[thread_attepmts_num * mod_t_global_id + (attempts + i) % thread_attepmts_num] * 8;
					remote_addr.offset =
						(thread_global_id * depth + i) *
						stride;
					update_ops++;
#ifdef HERD_LATENCY
					starts[i] = std::chrono::
