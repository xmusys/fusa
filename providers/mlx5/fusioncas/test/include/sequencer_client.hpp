	      ...
	      bool *stop_signal)
{
	...
	while ((*stop_signal) == false) {
		...
		struct ibv_sge sge;
		// we now run sequencer, so fix the address
		remote_addr.offset = 8;
#ifdef YCSB_LATENCY
		start = std::chrono::high_resolution_clock::now();
#endif
		// for bare apps, our rewrite turns faa into recv+write(unsignaled), so periodically flush sq
		// rdma read
		if (attempts % 16 == 0 && attempts != 0) {
		...
		uint64_t add = 1;
		// Sequencer use FAA
		coro_add_request(wr, sge, coro_id, IBV_WR_ATOMIC_FETCH_AND_ADD,
				 cmp_buf, remote_addr, rkey, remote_va,
				 sizeof(uint64_t), add, 0, IBV_SEND_SIGNALED,
				 conn->manager_);

		coro_sched->RDMASend(coro_id, qp, &wr, &bad_wr);
		coro_sched->Yield(yield, coro_id);
		// std::cout << "sequencer id: " << *(uint64_t *)(cmp_buf) << std::endl;
#ifdef YCSB_LATENCY
...
class SequencerClient : public BasicClient {
    public:
	uint64_t *keys;
	char *methods;
	double *latencies;
    ...
    public:
	SequencerClient(size_t nr_memory_nodes, size_t machine_num,
			   ...
			   double *tl_slot_latency)
	{
		...
		if (thread_id == report_thread) {
			SDS_INFO("SequencerClient Clinet report retry");
			...
			SDS_INFO("SequencerClient Clinet report Latencies");
