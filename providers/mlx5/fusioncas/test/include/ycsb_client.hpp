...
class YCSBClient : public BasicClient {
    public:
	uint64_t *keys;
	char *methods;

    public:
	YCSBClient(size_t nr_memory_nodes, size_t machine_num,
				  ...
				  uint64_t server_id, size_t depth) override
	{
		...
		*(buf) = 999;
		...
		cmp = 0;
		uint64_t swap = 0;
		std::random_device seed; // hardware random seed
		std::ranlux48 engine(seed()); // seed the rng engine

		std::uniform_int_distribution<> distrib(
			0, attempts_num); // uniform distribution range
		while (!stop_signal) {
			// while(attempts < thread_attepmts_num) {
			for (int i = 0; i < depth; i++) {
				...
				remote_addr.offset =
					(thread_global_id * depth + i) * 8;
				// remote_addr.offset = ((attempts_num / machine_num) * machine_id + (attempts + i) % (attempts_num / machine_num)) * 64;
				// remote_addr.offset = (thread_attepmts_num * thread_global_id + attempts + i) * 8;
				// remote_addr.offset = random * 8;
				// remote_addr.offset = keys[thread_attepmts_num * thread_global_id + attempts + i] * 8;
				// remote_addr.offset = thread_global_id * 4096;
				cmp = (attempts == 0 ?
					       0 :
					       (attempts / depth + i - 1));
				swap = attempts / depth + i;
				if (methods[attempts + i] == 'u') {
