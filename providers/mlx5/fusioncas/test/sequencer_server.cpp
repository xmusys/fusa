...
void SequencerServer::init_rpc(JsonConfig &rpc_config)
{
...
void *SequencerServer::rpc_func(void *rarg)
{
	uint64_t rpc_thread_id = ((thread_arg *)rarg)->thread_id;
	SequencerServer *server = ((thread_arg *)rarg)->server;
	// std::cout << "bbbbbbbbb" << std::endl;
	server->rpc_func_impl(rpc_thread_id);
}

void *SequencerServer::rpc_func_impl(uint64_t rpc_thread_id)
{
					...
					if (cur_req->crc == CRC_CHECK_MAGIC) {
						...
						// RPC FAA
							...
							t_local_id =
								cur_req->qpn %
								nr_qp;
							// FAA returns old value; cur_req->wr.atomic.cmp is the addend, so result = seq_id + cur_req->wr.atomic.cmp
							uint64_t sequence_id = FAA(
								(uint64_t *)cur_req
									->wr
									.atomic
									.remote_addr,
