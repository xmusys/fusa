...
{
	JsonConfig config =
		JsonConfig::load_file("../config/adaption_config.json");
	...
	if (node_type == "server") {
		...
		std::string second_path =
			client_config.get("second_path").get_str();
		std::string third_path =
			client_config.get("third_path").get_str();
		...
		AdaptionClient *client =
			new AdaptionClient(servers.size(), machine_num,
					   machine_id, depth, attempts_num);

		client->ycsb_load(ycsb_path);
		client->load_other_keys(second_path, third_path);
