...
#include "ht_server.h"
#include "cas_retry_client.hpp"

#define NUMA_NODE 1

int main(int argc, char **argv)
{
	JsonConfig config = JsonConfig::load_file("../config/backend.json");
	...
	if (node_type == "server") {
		...
		// memory node
		HTServer server(port, NUMA_NODE, mem_pool_size, rpc_region_size,
		...
		CASRetryClient *client =
			new CASRetryClient(servers.size(), machine_num,
