...
#include "server.h"
#include "smart_retry_client.hpp"

#define NUMA_NODE 1

int main(int argc, char **argv)
{
	JsonConfig config = JsonConfig::load_file("../config/smart_retry.json");
	...
	if (node_type == "server") {
		...
		SmartRetryClient *client =
			new SmartRetryClient(servers.size(), machine_num,
